...
public class CrossChainBitcoinResource {
    ...
    public org.qortal.api.model.crosschain.LocalWalletResponse publicSpendContext(org.qortal.api.model.crosschain.ForeignWalletRequest data) {
        ...
        try {
            ...
            return new org.qortal.api.model.crosschain.LocalWalletResponse(LocalWalletSupport.spendContext(Bitcoin.getInstance(), data.xpub58, data.expectedChainId));
    ...
    public org.qortal.api.model.crosschain.LocalWalletResponse publicTransactionStatus(org.qortal.api.model.crosschain.ForeignWalletRequest data) {
        ...
        try {
            if (data == null) throw new IllegalArgumentException();
            return new org.qortal.api.model.crosschain.LocalWalletResponse(LocalWalletSupport.transactionStatus(Bitcoin.getInstance(), data.txId, data.expectedChainId));
        } catch (IllegalArgumentException e) {
            throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_CRITERIA);
        } catch (ForeignBlockchainException e) {
            throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.FOREIGN_BLOCKCHAIN_NETWORK_ISSUE);
    ...
    public String broadcastSignedSpend(org.qortal.api.model.crosschain.ForeignWalletRequest data) {
        ...
        try {
            if (data == null) throw new IllegalArgumentException();
            return LocalWalletSupport.broadcast(Bitcoin.getInstance(), data.rawTransactionHex, data.expectedChainId);
        } catch (IllegalArgumentException e) {
            throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_CRITERIA);
        } catch (ForeignBlockchainException e) {
            throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.FOREIGN_BLOCKCHAIN_NETWORK_ISSUE);
	...
	public ForeignCoinStatus getWalletStatus() {
		...
		boolean isEnabled = btc != null;
		int connections = 0;
		int known = 0;
		if (isEnabled && btc.getBlockchainProvider() instanceof ElectrumX) {
			connections = ((ElectrumX) btc.getBlockchainProvider()).getConnectedServerCount();
			known = ((ElectrumX) btc.getBlockchainProvider()).getKnownServerCount();
	...
	@Path("/start")
	@Operation(
			summary = "Start Bitcoin Electrum Connections",
			description = "Start Bitcoin Electrum Connections",
			responses = {
					@ApiResponse(
							description = "true if Bitcoin Wallet Started",
							content = @Content(
									schema = @Schema(
											type = "string"
									)
